    ...
    public class OrderTokenProvider : ITokenProvider<Order>
    {
        ...
        private IDictionary<string, Func<Order, string>> GetTokens()
        {
            return new Dictionary<string, Func<Order, string>>
                {
                    {"CustomerName",order => order.User!=null?order.User.Name:(order.BillingAddress != null ? order.BillingAddress.Name : "Customer")},
                    {"CustomerFirstName",order => order.User!=null?order.User.FirstName:(order.BillingAddress != null ? order.BillingAddress.FirstName : string.Empty)},
                    {"CustomerLastName",order => order.User!=null?order.User.LastName:(order.BillingAddress != null ? order.BillingAddress.LastName : string.Empty)},
                    {"BillingAddressFormatted", order => order.BillingAddress.GetDescription()},
                    {"ShippingAddressFormatted", order => order.ShippingAddress.GetDescription()},
                    {"BillingAddressFormattedHtml", order => order.BillingAddress != null ? order.BillingAddress.GetDescription().Replace(",", ",<br />") : string.Empty},
                    {"ShippingAddressFormattedHtml", order => order.ShippingAddress != null ? order.ShippingAddress.GetDescription().Replace(",", ",<br />") : string.Empty},
