    ...
    public class SyncAmazonListingService : ISyncAmazonListingsService
    {
        ...
        public void ExportAmazonListing(AmazonSyncModel model)
        {
            ...
            if (amazonListing != null)
            {
                ...
                AmazonProgressBarHelper.Update(model.Task, "Push", "Creating Amazon Listings", 100, 0);

                _exportAmazonListingService.SubmitProductFeeds(model, amazonListing);

                AmazonProgressBarHelper.Update(model.Task, "Push", "Successfully created Amazon Listings",
                                               100, 100);
            }

            else
            {
                AmazonProgressBarHelper.Update(model.Task, "Push", "No listing to create", null, null);
            }
