...
namespace MrCMS.Helpers
{
    public static class SessionHelper
    {
            ...
            where TQuery : SystemEntity
        {
            ...
            IEnumerable<TResult> results = queryBase.Skip((pageNumber - 1) * size).Take(size).Future<TResult>();

            int rowCount = rowCountQuery.Select(countQuery).ClearOrders().FutureValue<int>().Value;
